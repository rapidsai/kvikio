...
CPP_CHANNEL=$(rapids-download-from-github "$(rapids-artifact-name conda_cpp libkvikio kvikio --cuda "$RAPIDS_CUDA_VERSION")")
PYTHON_CHANNEL=$(rapids-download-from-github "$(rapids-artifact-name conda_python kvikio kvikio --stable --cuda "$RAPIDS_CUDA_VERSION")")
