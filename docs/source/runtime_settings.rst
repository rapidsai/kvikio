...
KvikIO supports multiple libcurl-based backends for fetching data from remote endpoints (S3, HTTP, etc.). Set the environment variable ``KVIKIO_REMOTE_BACKEND`` to one of the following options (case-insensitive):
  ...
  * ``LIBCURL_MULTI_POLL``: Use libcurl's multi interface with poll-based concurrent transfers. A single call manages multiple concurrent connections using ``curl_multi_poll()``, with k-way buffering to overlap network I/O with host-to-device transfers. This can reduce thread overhead for high-connection-count scenarios.
