...
class BasicIOTest : public testing::Test {
 protected:
  void SetUp() override
  {
    ...
    _dev_a = std::move(DevBuffer::arange(1024 * 1024 * 100));
...
{
  {
    ...
    auto fut = f.pwrite(_dev_a.ptr, _dev_a.nbytes);
    fut.wait();
    EXPECT_EQ(fut.get(), _dev_a.nbytes);
  }

  {
    kvikio::FileHandle f(_filepath, "r");
    auto fut = f.pread(_dev_b.ptr, _dev_b.nbytes);
    fut.wait();
    EXPECT_EQ(fut.get(), _dev_b.nbytes);
...
//   std::array<kvikio::CompatMode, 2> compat_modes{kvikio::CompatMode::AUTO,
//   kvikio::CompatMode::ON}; for (auto const& compat_mode : compat_modes) {
//     {
