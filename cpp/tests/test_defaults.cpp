...
TEST(DefaultsTest, http_verbose)
{
  // Test default value (should be false)
  EXPECT_FALSE(kvikio::defaults::http_verbose());

  // Test setter/getter
  kvikio::defaults::set_http_verbose(true);
  EXPECT_TRUE(kvikio::defaults::http_verbose());

  kvikio::defaults::set_http_verbose(false);
  EXPECT_FALSE(kvikio::defaults::http_verbose());

  {
    ...
    for (const auto& value : true_values) {
      kvikio::test::EnvVarContext env_var_ctx{{"KVIKIO_HTTP_VERBOSE", value}};
      auto const [env_var_name, result, has_found] =
        kvikio::getenv_or({"KVIKIO_HTTP_VERBOSE"}, false);
      EXPECT_TRUE(result) << "Value '" << value << "' should be parsed as true";
    }
  }

  {
    ...
    for (const auto& value : false_values) {
      kvikio::test::EnvVarContext env_var_ctx{{"KVIKIO_HTTP_VERBOSE", value}};
      auto const [env_var_name, result, has_found] =
        kvikio::getenv_or({"KVIKIO_HTTP_VERBOSE"}, true);
      EXPECT_FALSE(result) << "Value '" << value << "' should be parsed as false";
    }
  }
